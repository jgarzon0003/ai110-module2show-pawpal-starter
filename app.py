...
if owner.pets:
    if st.button("Generate schedule"):
        scheduler = Scheduler()
        plan = scheduler.build_plan(owner, selected_pet)

        if plan:
            st.write(f"Schedule for {selected_pet.name}:")
            st.table(
                [
                    {
        ...
        skipped = [task for task in selected_pet.tasks if task.start_time is None]
        if skipped:
